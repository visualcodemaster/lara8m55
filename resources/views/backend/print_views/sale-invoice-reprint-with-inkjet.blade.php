...
<main>
    <table style="border: 1px solid">
        ...
        <tbody>
        @foreach($saleInvoice->saleDetail as $cart)
            <tr style="border: 1px solid">
                <td width="50%" style="word-break: keep-all; " class="service">{{$cart->products->product_name}}</td>
                <td width="15%" class="qty">{{$cart->product_qty}}</td>
                <td width="15%" class="unit">{{$cart->price}}</td>
                <td width="20%" class="total">{{$cart->total_price}}</td>
            </tr>
        @endforeach
        </tbody>
    </table>
    ...
    <table style="text-align: center">
        <tr>
            ...
            <td width="35%" class="f20">{{count($saleInvoice->saleDetail)}}</td>
            <th width="25%" class="f20">Previous</th>
            <td width="25%" class="f20">{{$saleInvoice->previous}}</td>
        </tr>
        <tr>
            <th width="15%" class="f20">Amount</th>
            <td width="35%" class="f20">{{$saleInvoice->amount}}</td>
            <th width="25%" class="f20">Receivable</th>
            <td width="25%" class="f20">{{$saleInvoice->total_amount}}</td>
        </tr>
        <tr>
            <th width="15%" class="f20">Discount</th>
            <td width="35%" class="f20">{{$saleInvoice->discount}}</td>
            <th width="25%" class="f20">Received</th>
            <td width="25%" class="f20">{{$saleInvoice->received}}</td>
        </tr>
        <tr>
            <th width="15%" class="f20">Net</th>
            <td width="35%" class="f20">{{$saleInvoice->net_amount}}</td>
            <th width="25%" class="f20">Balance</th>
            <td width="25%" class="f20">{{$saleInvoice->balance}}</td>
        </tr>
    </table>
    <hr>
    <footer>
